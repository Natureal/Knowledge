...
参考的有价值的博文：https://www.cnblogs.com/loveis715/p/4681643.html

在学习 Memcached 的过程中，发现其融合了许多经典的思想和结构：（1）slab 内存管理 （2）LRU \& Lazy expiration 算法（3）一致性哈希算法

1. slab 内存管理

目的：处理内存碎片化问题。

将内存分成默认大小为 1MB 的页，其内部会分成一系列具有相同大小的块。不同页的块的大小可能不同，并且有默认为 2 的递增倍率（如 50KB, 100KB, 200KB ...）

2. LRU \& Lazy expiration 算法

过期的数据在下一次写入数据却没有足够空间时，会被覆盖。此时，如果没有过期数据，才会调用 LRU 算法替换掉最久没有被使用的数据。

Plus：LRU 是在同大小的 slab 块之间的，比如写入 100K 的数据但没空间也没数据过期时，会找一个 100K 的块去替换。

3. 一致性哈希算法

在 memcached 的每条记录都有四个部分：键值，有效期，一系列可选的标记，数据

数据的键值在经过一致性哈希后会和某一个 memcached 实例对应。这使得数据分布式地存储，当一个实例挂了，其他实例依然工作（并支持转移缓存），提高了可用性。
